...
def test_apply_json_patch(dummy_json_save: Path) -> None:
    ...
    from gamegenie_x.profiles import PlatformProfile, IOStrategy, FieldDef
    ...
    with open(dummy_json_save, "r") as f:
        data = json.load(f)
    assert data["player"]["currency"] == 0xFF

    # Apply increment patch on level
    patch2 = Patch(address=0x2000, value=0x01, patch_type=PatchType.INCREMENT, platform=Platform.UNIVERSAL)
...
def test_apply_json_patch_compare_fail(dummy_json_save: Path) -> None:
    ...
    patch = Patch(address=0x1000, value=0xFF, compare=99, platform=Platform.UNIVERSAL, flags=Flags(compare_enabled=True))
...
def test_fuzz_binary_patching(dummy_save_file: Path) -> None:
    ...
    for _ in range(50):
        ...
        ptype = random.choice([PatchType.REPLACE, PatchType.INCREMENT, PatchType.DECREMENT, PatchType.XOR, PatchType.BITSET, PatchType.BITCLEAR])
